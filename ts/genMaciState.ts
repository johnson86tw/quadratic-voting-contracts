...
/**
 * Notice that this file is different from original maci code, and
 * it's only for scripts/tallyVotesWithoutMerge.ts
 */

import { ethers } from "ethers";
import { Keypair, PubKey, Message } from "maci-domainobjs";
import { MaciState } from "./maciState";
import { MACI__factory, Poll__factory } from "../build/typechain";
...
const genMaciState = async (
  ...
  const pollContractAbi = Poll__factory.abi;
  const maciContractAbi = MACI__factory.abi;
  ...
  for (const log of signUpLogs) {
    ...
    actions.push({
      ...
      data: {
        ...
        pubKey: new PubKey(event.args._userPubKey.map((x: any) => BigInt(x))),
  ...
  for (const log of deployPollLogs) {
    ...
    const pubKey = new PubKey(
      event.args._pubKey.map((x: any) => BigInt(x.toString()))
    );
  ...
  for (const log of publishMessageLogs) {
    ...
    const encPubKey = new PubKey(
      event.args._encPubKey.map((x: any) => BigInt(x.toString()))
    );
